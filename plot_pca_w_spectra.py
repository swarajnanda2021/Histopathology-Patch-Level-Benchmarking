#!/usr/bin/env python3
"""
PCA Visualization with Integrated Polar Plots - Consensus Masking Version
Updated: Uses exact same PCA calculation procedure as plot_pcaviz.py
Now with optional subplot highlighting and white background
Font sizes optimized for ICLR A4 two-column format (with +2 point buffer)
"""
...
def find_optimal_threshold_otsu(values):
    ...
    # Normalize values to 0-255 range for Otsu
...
def validate_whitespace_separation(pca_component, patches, threshold):
    """Validate if PCA component separation corresponds to actual whitespace"""
    # Create mask based on threshold
    mask = pca_component > threshold
    
    # Calculate actual brightness for each token position
    actual_brightness = []
    
    for patch in patches:
        # Divide each 224x224 patch into 14x14 tokens (16x16 pixels each)
        for i in range(14):
            for j in range(14):
    ...
    # Check if high PCA values correspond to bright pixels
    bright_threshold = 240
    actual_white_mask = actual_brightness > bright_threshold
    
    # Calculate correlation metrics
    correlation = np.corrcoef(pca_component, actual_brightness)[0, 1]
    
    # Mask agreement (Jaccard index)
    if np.sum(actual_white_mask) > 0:
        intersection = np.sum(mask & actual_white_mask)
        union = np.sum(mask | actual_white_mask)
        jaccard = intersection / union if union > 0 else 0
    else:
        jaccard = 0
    
    # Validate
    is_valid = (abs(correlation) > 0.5) or (jaccard > 0.5)
    
    # If negative correlation, flip the mask
    if correlation < -0.5:
        mask = ~mask
        correlation = -correlation
    
    print(f"    Validation: correlation={correlation:.3f}, jaccard={jaccard:.3f}, valid={is_valid}")
    
    return mask, correlation, is_valid

def direct_whitespace_detection(patches):
    """Directly detect whitespace from original patches"""
    print("  Direct whitespace detection from original pixels...")
    
    brightness_values = []
    
    for patch in patches:
        # Process each 16x16 token in the patch
        for i in range(14):
            for j in range(14):
                region = patch[i*16:(i+1)*16, j*16:(j+1)*16]
                gray_value = np.mean(np.dot(region, [0.299, 0.587, 0.114]))
    ...
    # Use Otsu on brightness values
    threshold = find_optimal_threshold_otsu(brightness_values)
    background_mask = brightness_values > threshold
    
    n_background = np.sum(background_mask)
    n_total = len(background_mask)
    print(f"    Direct detection: {n_background}/{n_total} ({n_background/n_total*100:.1f}%) marked as whitespace")
    
    return background_mask, brightness_values

def evaluate_separation_quality(mask, patches, method_name):
    """Evaluate how well a separation mask preserves tissue while removing whitespace"""
    # Calculate actual brightness for each token
    actual_brightness = []
    ...
    # Define what is actual whitespace vs tissue
    actual_whitespace = actual_brightness > 240
    actual_tissue = actual_brightness < 200
    
    # Calculate metrics
    true_positives = np.sum(mask & actual_whitespace)
    false_positives = np.sum(mask & actual_tissue)
    false_negatives = np.sum(~mask & actual_whitespace)
    
    # Calculate rates
    n_whitespace = np.sum(actual_whitespace)
    n_tissue = np.sum(actual_tissue)
    
    whitespace_recall = true_positives / n_whitespace if n_whitespace > 0 else 0
    tissue_removed_rate = false_positives / n_tissue if n_tissue > 0 else 0
    
    # Quality score: high whitespace removal, low tissue removal
    quality_score = whitespace_recall - 2.0 * tissue_removed_rate
    
    metrics = {
        'whitespace_recall': whitespace_recall,
        'tissue_removed_rate': tissue_removed_rate,
    ...
    print(f"      - Tissue incorrectly removed: {tissue_removed_rate:.1%}")
...
def compute_foreground_mask(features, patches):
    ...
    # Use 4 clusters: background, light tissue (vessels/fat), medium tissue, dark tissue
    n_clusters = 4
    ...
    for cluster_id in range(n_clusters):
        ...
        if len(cluster_pixels) > 0:
            # Get brightness statistics for this cluster
            ...
            # Compute spatial coherence (are pixels connected?)
            # Reshape cluster mask to 2D grid for spatial analysis
            cluster_mask_2d = cluster_mask.reshape(256, 196).any(axis=1).reshape(16, 16)
            labeled, n_components = ndimage.label(cluster_mask_2d)
            
            # Check if pixels are at periphery (for 16x16 patch grid)
    ...
    for stats in cluster_stats:
        ...
        # High brightness is strong indicator of background
    ...
    # Only consider clusters with high confidence scores
    background_threshold_score = 4  # Require at least 4 points of evidence
    ...
    if not background_clusters:
        ...
        # Take the highest scoring cluster if it's reasonably bright
    ...
    # Stage 8: Additional validation - check if we're removing actual tissue
    # If we're removing pixels with brightness < 200, be more conservative
...
def plot_polar_with_normalization(ax, hue_data, model_type):
    """Polar plot with individual normalization and color ring"""
    filtered_data = {k: v for k, v in hue_data.items() 
                     if model_type in k and v is not None and len(v) > 0}
    
    if not filtered_data:
        return
    
    aug_colors = {'standard': '#1E88E5', 'masked': '#DC3545', 'mixed': '#FFA726'}
    ...
    for model_name, hue_values in filtered_data.items():
        aug_type = 'standard' if 'standard' in model_name else 'masked' if 'masked' in model_name else 'mixed'
        hue_radians = hue_values * np.pi / 180
        counts, _ = np.histogram(hue_radians, bins=theta_edges)
        # REMOVED SMOOTHING: counts = gaussian_filter1d(counts.astype(float), sigma=1.0, mode='wrap')
        ...
        ax.plot(theta_plot, counts_plot, color=aug_colors[aug_type], linewidth=2.5, alpha=0.9)
        ax.fill(theta_plot, counts_plot, color=aug_colors[aug_type], alpha=0.15)
    ...
    # Radial axis - make less prominent (CORRECTED FONT SIZE: 11)
    ax.set_ylim(0, 1.25)  # Extended to accommodate color ring
    ...
    for i, angle in enumerate(theta_ring):
        ...
        # Since theta_zero is North and clockwise, adjust accordingly
        ...
        # Draw a thin radial bar for this segment
    ...
    # Augmentation type labels (CORRECTED FONT SIZE: 13)
    label_positions = {
        'standard': (0.75, 0.55),  # bottom-right
        'masked': (0.75, 0.45),     # below standard
        'mixed': (0.75, 0.35)       # below masked
    }
...
def create_final_figure(original_image, all_rgb_grids, all_hue_data, output_path, highlight_subplot=None):
    """Create final integrated figure with proper aspect ratio to eliminate gaps
    
    Args:
        original_image: The original 3584x3584 region
        all_rgb_grids: Dictionary of RGB grids for each model
        all_hue_data: Dictionary of hue values for each model
        output_path: Path to save the figure
        highlight_subplot: Name of subplot to highlight (e.g., 'DINOv2: mixed augmentations')
                          or None for no highlighting
    """
    
    # Calculate dimensions for square PCA plot cells
    # Width ratios: [2, 1, 1, 1, 0.8] = total 5.8 units
    # Heights: 2 rows
    
    # Set figure width
    fig_width = 20  # inches
    
    # Calculate width of PCA columns (cols 1-3 each get 1/5.8 of width)
    width_ratios = [2, 1, 1, 1, 0.8]
    total_width_units = sum(width_ratios)
    pca_col_width_inches = fig_width * (1 / total_width_units)
    
    # Since PCA plots need to be square, and we have 2 rows,
    # figure height should be 2 * pca_col_width_inches
    fig_height = 2 * pca_col_width_inches * 1.05  # 1.05 for minimal padding
    
    # Create figure with WHITE background (changed from '#f8f8f8')
    fig = plt.figure(figsize=(fig_width, fig_height), facecolor='white')
    
    # Grid with minimal spacing
    gs = fig.add_gridspec(2, 5, 
                          hspace=0.02,  # Minimal horizontal spacing
                          wspace=0.08,  # Slightly increased from 0.05 for polar column
    ...
    # Original image spanning both rows (CORRECTED FONT SIZES)
    ax = fig.add_subplot(gs[:, 0])
    ax.imshow(original_image, extent=[0, 3584, 3584, 0], interpolation='nearest')
    ax.set_xlabel('Pixels', fontsize=14)  # Corrected: 12+2
    ax.set_ylabel('Pixels', fontsize=14)  # Corrected: 12+2
    ax.set_xticks([0, 1792, 3584])
    ax.set_yticks([0, 1792, 3584])
    ax.tick_params(axis='both', labelsize=13)  # Corrected: 11+2
    
    # Add label as text box inside plot (CORRECTED FONT SIZE)
    ax.text(0.02, 0.98, f'{labels[idx]} Original Image (3584×3584)',
           transform=ax.transAxes,
           fontsize=15, fontweight='bold',  # Corrected: 13+2
    ...
    # PCA plots
    model_positions = {
        'DINOv1: standard augmentations': (0, 1, 'DINOv1', 'standard'),
        'DINOv1: masked augmentations': (0, 2, 'DINOv1', 'masked'),
        'DINOv1: mixed augmentations': (0, 3, 'DINOv1', 'mixed'),
        'DINOv2: standard augmentations': (1, 1, 'DINOv2', 'standard'),
        'DINOv2: masked augmentations': (1, 2, 'DINOv2', 'masked'),
        'DINOv2: mixed augmentations': (1, 3, 'DINOv2', 'mixed'),
    }
    
    for model_name, (row, col, base, aug) in model_positions.items():
        if model_name in all_rgb_grids:
            ...
            # Add label as text box inside plot (CORRECTED FONT SIZE)
            ax.text(0.02, 0.98, f'{labels[idx]} {base} ({aug})',
                   transform=ax.transAxes,
                   fontsize=15, fontweight='bold',  # Corrected: 13+2
                   verticalalignment='top',
                   bbox=dict(boxstyle='round,pad=0.3',
    ...
    if highlight_subplot and highlight_subplot in axes_dict:
        ...
        # Create a rectangle patch with dashed line
        highlight_rect = Rectangle(
            (bbox.x0 - 0.005, bbox.y0 - 0.005),  # Small offset to encompass borders
            bbox.width + 0.01, 
            bbox.height + 0.01,
    ...
    # Polar plots - adjusted to fit within the properly sized grid
    # Top polar plot
    ax_polar1 = fig.add_subplot(gs[0, 4], projection='polar')
    plot_polar_with_normalization(ax_polar1, all_hue_data, 'DINOv1')
    
    # Add label for DINOv1 polar (CORRECTED FONT SIZE)
    ax_polar1.text(0.5, 1.15, f'{labels[idx]} DINOv1',
                  transform=ax_polar1.transAxes,
                  fontsize=15, fontweight='bold',  # Corrected: 13+2
                  ha='center',
                  bbox=dict(boxstyle='round,pad=0.3',
                           facecolor='white',
    ...
    # Bottom polar plot
    ax_polar2 = fig.add_subplot(gs[1, 4], projection='polar')
    plot_polar_with_normalization(ax_polar2, all_hue_data, 'DINOv2')
    
    # Add label for DINOv2 polar (CORRECTED FONT SIZE)
    ax_polar2.text(0.5, 1.15, f'{labels[idx]} DINOv2',
                  transform=ax_polar2.transAxes,
                  fontsize=15, fontweight='bold',  # Corrected: 13+2
                  ha='center',
                  bbox=dict(boxstyle='round,pad=0.3',
                           facecolor='white',
...
def process_region(slide, region_info, region_index, checkpoints, device, output_dir, cancer_type):
    """Process single region with consensus masking"""
    ...
    # PHASE 1: Extract features and compute individual masks
    print("PHASE 1: Feature extraction and individual mask computation")
    ...
    # Create figures - both with and without highlighting
    if all_rgb_grids:
        # Version without highlighting (for main paper)
        output_path = os.path.join(output_dir, f"{cancer_type}_{region_index}.png")
        create_final_figure(original_image, all_rgb_grids, all_hue_data, output_path, 
        ...
        create_final_figure(original_image, all_rgb_grids, all_hue_data, output_path_highlighted,
...
def main():
    base_output_dir = "pca_visualizations_with_polar_3"
    ...
    checkpoints = {
        'DINOv1: standard augmentations': '/data1/vanderbc/nandas1/TCGA_Dino_ViT-B_run2/logs/checkpoint.pth',
        'DINOv1: masked augmentations': '/data1/vanderbc/nandas1/TCGA_TMEDinov1_version3_ViT-B/logs/checkpoint.pth',
        'DINOv1: mixed augmentations': '/data1/vanderbc/nandas1/TCGA_TMEDinov1_version2_ViT-B/logs/checkpoint.pth',
    ...
    for cancer_type, wsi_path in wsi_paths.items():
        ...
        process_wsi(wsi_path, cancer_type, cancer_dir, checkpoints, device, n_regions=10)
