...
def create_unified_patch_benchmark_plot(comparisons, base_path, output_dir):
    ...
    model4_metrics = load_all_metrics(comp['model4_path']) if os.path.exists(comp['model4_path']) else []
    ...
    for col_idx, dataset in enumerate(datasets):
        ...
        if is_segmentation[col_idx]:
            ...
            if os.path.exists(comp['model4_path']):
                stats4, name4 = load_segmentation_data(comp['model4_path'],
                                                       comp['model4_name'],
                                                       'PanNuke')
                if stats4 is not None:
                    data_dict[name4] = stats4
            ...
            if model4_metrics:
                df4 = extract_classification_data(model4_metrics, dataset, 'auc')
                if not df4.empty:
                    data_dict[comp['model4_name']] = df4
    ...
    # Add legend with four augmentation types
    if legend_lines:
        handles = []
        labels = []
        for aug_type in ['standard', 'masked', 'mixed', 'random_masking']:
            if aug_type in legend_lines:
                handles.append(legend_lines[aug_type][0])
                labels.append(legend_lines[aug_type][1])
        
        fig.legend(handles, labels, loc='upper center', bbox_to_anchor=(0.5, 1.05),
                  ncol=4, fontsize=16, frameon=True, fancybox=False,
    ...
    output_path = os.path.join(output_dir, "patch_benchmarks_dinov2_rebuttal.png")
...
def main():
    ...
    comparisons = [
        ...
        },
        {
            ...
            'model4_path': os.path.join(base_path, "TCGA_TMEDinov2_version4_random_masking_ViT-B"),
            'model1_name': 'Dinov2_ViT-B',
            'model2_name': 'TMEDinov2_ViT-B_version2',
            'model3_name': 'TMEDinov2_ViT-B_version3',
            'model4_name': 'TMEDinov2_ViT-B_random_masking',
            'ssl_method': 'Dinov2'
        }
    ]
